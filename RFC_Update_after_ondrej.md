...
A new **Admin → Setup → LDAP** area follows the **Satellite 6 LDAP authentication source** layout[^8]: when editing a source, the administrator sees four tabs — **Server** (host, port, transport, directory type), **Account** (bind credentials, base DNs, provisioning), **Attribute mappings** (login/name/email attributes, pre-filled per server type), and **User Groups** (external group → Uyuni role bindings). This is the same structure demonstrated in the Satellite LDAP walkthrough video and is reused here so administrators already familiar with Red Hat Satellite need no new mental model.

Three test actions map to POC operations: test connection, test user lookup, test group resolution.
