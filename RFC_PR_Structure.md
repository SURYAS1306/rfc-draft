...
│   └── images/
│       └── native-ldap-login-flow.png         ← optional; only if not rendering mermaid inline
└── (do not edit 00000-template.md)
```

The RFC keeps its diagrams as inline mermaid (GitHub renders mermaid in markdown), so the `images/` directory is optional. Add a PNG only if reviewers prefer a static image of the login sequence.
...
See **Unresolved questions** in the RFC (six design-level items). Feedback most
welcome on secret/trust-material handling, scoping external groups to a server,
superuser reachability, and the JIT org-assignment default.
...
| Login-flow PNG (only if not using inline mermaid) | `accepted/images/native-ldap-login-flow.png` |

## Diagram export (optional)

The RFC keeps its one mermaid sequence diagram (the Local → LDAP → PAM login
flow) inline, which GitHub renders directly. Only if a reviewer asks for a static
image, export that single diagram (~1200px wide) and swap the mermaid block for:

```markdown
![login flow](images/native-ldap-login-flow.png)
```

Tools: VS Code Mermaid export or `mmdc` (mermaid-cli).
...
- Detailed design at the right level: data model described conceptually (entities and rationale, not finalized DDL), Java integration points, UI/API
- Honest drawbacks and alternatives (architecture-level, with implementation choices merged)
