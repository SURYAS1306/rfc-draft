...
- **Bind account** - the service-account bind DN (null meaning anonymous bind) and a reference to the stored bind password. Depending on the credential-storage decision below, the bind DN is held either on this entity or as the `username` of the linked `suseCredentials` row.
...
1. **Secret and trust material** - is Base64-at-rest (as for SCC/registry credentials) acceptable for the bind password, or is encryption required; should the bind DN be stored as the credential `username` (keeping the existing `suseCredentials` pair shape) or kept on `suseLdapAuthServer` with a relaxed `cred_type_check` branch; and where should the directory CA live: the JVM trust store, the container's `/etc/pki/trust/anchors/`, or an upload-through-UI flow?
...
7. **Identity collisions** - login order is local-first, so a local user shadows an LDAP user with the same login, and an LDAP user whose login matches an existing local account cannot authenticate via LDAP. Is shadowing the intended behavior, or should collisions be rejected or namespaced?
8. **Empty / unauthenticated bind** - many directories treat a bind with a valid DN and an empty password as a successful unauthenticated bind. The provider must reject empty passwords before binding; is that the agreed safeguard, and are there other bind edge cases (for example, disabled or expired accounts) to handle explicitly?
9. **Active Directory group retrieval** - AD returns membership in ranges (`member;range=0-1499`) for large groups, and the `memberOf` path is untested so far. Are `member;range=` handling and `memberOf` in scope for v1, or deferred with documented limits?
